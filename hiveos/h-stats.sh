...
stats_raw=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstat1"}' | nc -w $API_TIMEOUT localhost $WEB_PORT | jq '.result'`
if [[ $? -ne 0  || -z $stats_raw ]]; then
	echo -e "${YELLOW}Failed to read $miner stats_raw from localhost:${WEB_PORT}${NOCOLOR}"
else
	...
	[[ ! -z $CUSTOM_ALGO ]] && local algo=$CUSTOM_ALGO || local algo="ethash"
